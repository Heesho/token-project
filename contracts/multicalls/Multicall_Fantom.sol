...
contract Multicall_Fantom {
    ...
    function gaugeCardData(address plugin, address account, uint256 price) public view returns (GaugeCard memory gaugeCard) {
        ...
        gaugeCard.priceUnderlying = price;
        gaugeCard.priceOTOKEN = ITOKEN(TOKEN).getOTokenPrice() * (uint256(IOracle(PRICE_ORACLE_BASE).latestAnswer()) * 1e18 / 1e8) / 1e18;
    ...
    function getGaugeCards(uint256 start, uint256 stop, address account, uint256[] memory prices) external view returns (GaugeCard[] memory) {
        GaugeCard[] memory gaugeCards = new GaugeCard[](stop - start);
        for (uint i = start; i < stop; i++) {
            gaugeCards[i] = gaugeCardData(getPlugin(i), account, prices[i]);
        }
        return gaugeCards;
    }
